...
Free Tier dokumentiert bis zu 5000 Kerzen pro Request, also ~20 Jahre
Historie in **einem** Request statt Alpha Vantages künstlicher
100-Tage-Deckelung.

**Split-Bereinigung: Twelve Data macht das schon selbst.** Live verifiziert
am 2026-08-10, nachdem ein eigener manueller Bereinigungs-Layer (siehe unten
"Wie wir das herausgefunden haben") die Kurse fuer Ticker mit grossen Splits
kaputt gerechnet hatte: der von `time_series` zurueckgegebene `close`-Wert
ist bereits split-bereinigt, obwohl der Endpunkt nicht "adjusted" heisst.
Beweis: NVDAs gespeicherter `close` fuer 2024-06-07 (letzter Handelstag vor
dem 10:1-Split) war `120.888` — exakt der reale unbereinigte Schlusskurs von
$1208,88 geteilt durch 10. Deshalb wird `close` direkt als `adjusted_close`
uebernommen (`src/twelvedata.ts`, `src/cron.ts`), **ohne** eine zweite,
eigene Bereinigung obendrauf. `/splits` als eigener Endpunkt existiert bei
Twelve Data zwar, erfordert aber nachweislich einen bezahlten Plan
(Fehlermeldung: "available exclusively with grow or pro or ultra or venture
or enterprise plans") und wird nicht gebraucht.

> **Wie wir das herausgefunden haben** (fuer kuenftige Aenderungen an dieser
> Logik relevant): eine fruehere Version dieses Projekts ging davon aus,
> Twelve Data liefere unadjusted Kurse (wie Alpha Vantage es fuer den
> Nicht-Premium-Endpunkt tut) und wandte zusaetzlich eine eigene,
> manuell kuratierte Split-Tabelle an (`scripts/seed-splits.sql`,
> `src/splitAdjustment.ts`, per Cron nach jedem Fetch aufgerufen). Das
> bereinigte historische Kurse **doppelt** und produzierte einen Backtest
> mit ~99 % annualisierter Volatilitaet und einem ploetzlichen, falschen
> Sprung in der Equity-Kurve genau an den Split-Daten. Der Fix: die eigene
> Bereinigung komplett entfernt (`src/db.ts` hat noch die Funktionen
> `applySplitAdjustment`/`getSplits` als unbenutzte Referenz, falls ein
> kuenftiger Datenanbieter tatsaechlich unadjusted Kurse liefert) und einen
> Reparatur-Endpunkt ergaenzt (`POST /api/admin/reset-adjusted-close`, auch
> als Dashboard-Button "Adjusted-Close-Korrektur zuruecksetzen"), der
> `adjusted_close` wieder auf den rohen `close`-Wert zuruecksetzt.
>
> **Falls hier je wieder ein aehnlicher Fehler auftaucht** (unrealistisch
> hohe Volatilitaet/CAGR, ploetzlicher Sprung in der Equity-Kurve genau an
> einem bekannten Split-Datum): vor jeder Aenderung an der
> Adjustierungs-Logik den gleichen Beweis fuehren, den wir hier gefuehrt
> haben — echten historischen Schlusskurs eines Tickers kurz vor einem
> bekannten Split nachschlagen und mit dem gespeicherten `close`-Wert
> vergleichen (`SELECT date, close FROM prices WHERE ticker='...' AND date
> BETWEEN '<vor dem Split>' AND '<nach dem Split>' ORDER BY date;` in der
> D1-Console).

**Wichtige verbleibende Einschraenkung:** soweit bekannt ist das nur
**split-bereinigt, nicht dividenden-bereinigt** (nicht live verifiziert,
aber Standard-Konvention bei den meisten Datenanbietern fuer den
"plain close"). Fuer die Mega-Cap-Watchlist bedeutet das eine systematische
Unterschaetzung des Total Return um grob 1-2 %/Jahr (Dividendenrendite), UND
zwar gleichermassen fuer Strategie- wie fuer Benchmark-Ticker, was den
relativen Vergleich (CAGR-Differenz) weniger verzerrt als das absolute CAGR
selbst. Momentum-Ranking (12-1-Return) ist davon nur marginal betroffen, da
...
auch nur ein paar Minuten. Danach ist sofort mehrjaehrige, bereits
split-bereinigte Historie fuer Screener und Backtest da (siehe
"Datenquelle" oben — keine eigene Bereinigung noetig). Details siehe
`src/cron.ts`.
...
| `/api/admin/test-fetch` | GET | roher Twelve-Data-Testaufruf, `?function=daily\|splits` (schreibt nichts in D1; `splits` braucht einen bezahlten Plan und wird nicht fuer die Bereinigung gebraucht, siehe "Datenquelle") |
| `/api/admin/run-update` | POST | Preis-Update manuell ausloesen (= Cron-Logik) |
| `/api/admin/reset-adjusted-close` | POST | Reparatur-Tool: setzt `adjusted_close` fuer **alle** Watchlist-Ticker auf den rohen `close`-Wert zurueck, ohne neue Kursdaten zu holen (kein Twelve-Data-Request) — auch als Button "Adjusted-Close-Korrektur zuruecksetzen" im Dashboard. Nur noetig, falls jemals eine (falsche) zusaetzliche Bereinigung angewendet wurde, siehe "Datenquelle". |
...
Sharpe/CAGR/Max-Drawdown-Formeln gegen geschlossene Loesungen, dass der
...
Symbol) gegen das dokumentierte Antwortschema. (`splitAdjustment.ts`s
Bereinigungs-Mathematik hat ebenfalls einen Test, auch wenn die Funktion im
Standard-Pfad aktuell nicht mehr aufgerufen wird — siehe "Datenquelle".)
...
bereits split-bereinigt — siehe "Datenquelle"-Abschnitt oben). Bitte die
Nutzungsbedingungen von Twelve Data beachten, insbesondere bei Weitergabe
der Daten.
